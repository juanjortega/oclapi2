...
class ConceptDocument(Document):
    ...
    all_names = fields.ListField(fields.KeywordField())
    ...
    @staticmethod
    def prepare_all_names(instance):
        return list(
            instance.names.filter(name__isnull=False).distinct('name').values_list('name', flat=True)
        )
